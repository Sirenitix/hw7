...
(function ($) {
        ...
        $("#search").click(function () {
            ...
            if(errmessage === "ru") {
                ...
                if (searchVal == '') {
                    $("#header-search").after('<h4 align="center" class="error">Пожалуйста, введите значение для поиска.</h4>');
                    hasError = true;
                } else if (!searchReg.test(searchVal)) {
                    $("#header-search").after('<h4  align="center" class="error">Недопустимый текст.</h4>');
                    hasError = true;
                }
                if (hasError == true) {
                    return false;
                }
            }else if(errmessage === "en"){
                $(".error").hide();
                var hasError = false;
                var searchReg = /^[a-zA-Z0-9-]+$/;
                var searchVal = $("#query").val();
                if (searchVal == '') {
                    $("#header-search").after('<h4 align="center" class="error">Please enter a search term.</h4>');
                    hasError = true;
                } else if (!searchReg.test(searchVal)) {
                    $("#header-search").after('<h4  align="center" class="error">Enter a valid text. </h4>');
                    hasError = true;
                }
                if (hasError == true) {
                    return false;
    ...
    $('.a-genre').each(function(){
        ...
        $this.attr("href", _href + '/' + $(this).text() + '/' + _id);
    })
    ...
    $(function () {
        ...
        var Card = function () {
            var bookTemplate = function (book) {
                return '<div class="Card">' + '   <div class="Card-picture">' + '<a href="/books/slug">' + '       <img src="' + book.image + '">\n' + '</a>' +
                    ...
                    (book.status === 'KEPT' ? ' <a class="Card-status" title="Отложена"><img src="/assets/img/icons/heart.svg" alt="Отложена"></a>\n' : '') + '    </div>' + '              <div class="Card-content">\n' + '                <strong class="Card-title"><a href="/books/slug">' + book.title + '</a>\n' + '                </strong>\n' + '                <div class="Card-description">' + book.authors[0].name + '                </div>\n' + '                <div class="Card-cost">' +
                    (book.discount ? '<span class="Card-priceOld">₽' + book.discount + '</span>\n' : '') + '                       <span class="Card-price">₽' + book.price + '</span>\n' + '                </div>\n' + '              </div>\n' + '            </div>';
            }
            return {
                init: function () {
                                ...
                                getData('/books/page/genre', {
                                    ...
                                    id: function () {
                                        ...
                                        $.urlParam = function(){
                                            ...
                                            console.log(results[0].replace("/", ""));
                                            return results[0].replace("/", "");
                                        }
                                        $.urlParam('id');
                                        return  decodeURIComponent($.urlParam('id'))
                                        //
                                    }
                                });
                                break;
                            case'author':
                                getData('/books/page/author/', {
                                    offset: function () {
                                        var oldOffset = $this.data('refreshoffset');
                                        if(oldOffset < 1){
                                            $this.data('refreshoffset', ++oldOffset);
